...
class App:
    ...
    def on_reset_screen(self):
        """
        Resets  and reloads the window manager and the browser.
        """
        self.__screen.reload()
    ...
    def on_before_request(self):
        ...
        if request.endpoint in ['on_get_index','on_login','on_is_authenticated','on_logout', 'on_get_resource']:
            return None
    ...
    def run(self):
        ...
        app.add_url_rule('/screen/screenshot.png', view_func=self.on_get_screenshot, methods=['GET'])
        app.add_url_rule('/screen/on',  view_func=self.on_set_screen_on, methods=['GET'])
        app.add_url_rule('/screen/off',  view_func=self.on_set_screen_off, methods=['GET'])
        app.add_url_rule('/screen',  view_func=self.on_get_screen, methods=['GET'])
        app.add_url_rule('/screen',  view_func=self.on_set_screen, methods=['POST'])

        app.add_url_rule('/cert', view_func=self.on_get_cert, methods=["GET"])
        app.add_url_rule('/cert', view_func=self.on_set_cert, methods=["POST"])
        app.add_url_rule('/cert/generate', view_func=self.on_generate_cert, methods=["POST"])

        app.add_url_rule('/schedule', view_func=self.on_get_schedule, methods=["GET"])
        app.add_url_rule('/schedule', view_func=self.on_set_schedule, methods=["POST"])
